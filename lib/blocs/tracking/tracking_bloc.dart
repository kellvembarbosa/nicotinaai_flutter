...
class TrackingBloc extends Bloc<TrackingEvent, TrackingState> {
  ...
  }) : _repository = repository, super(const TrackingState()) {
  ...
  Future<void> _onDeleteSmokingLog(DeleteSmokingLog event, Emitter<TrackingState> emit) async {
    try {
      ...
      // Update the local list
      final updatedLogs = state.smokingLogs
        ...
        if (event.craving.outcome == CravingOutcome.resisted) {
          ...
          if (state.userStats != null) {
            ...
            await AnalyticsService().setUserProperties(
              daysSmokeFree: state.userStats!.smokeFreeStreak,
            );
          }
        }
  ...
  Future<void> _onUpdateCraving(UpdateCraving event, Emitter<TrackingState> emit) async {
    try {
      ...
      // Update the local list
      final updatedCravings = state.cravings.map((c) {
  ...
  Future<void> _loadHealthRecoveries(Emitter<TrackingState> emit, {bool forceRefresh = false}) async {
    ...
    if (!forceRefresh && !_isCacheExpired(_lastRecoveriesUpdateTime) && 
        state.healthRecoveries.isNotEmpty && state.userHealthRecoveries.isNotEmpty) {
      if (kDebugMode) {
  ...
  void _onClearError(ClearError event, Emitter<TrackingState> emit) {
    if (state.hasError) {
      emit(state.copyWith(
        status: state.isLoaded ? TrackingStatus.loaded : TrackingStatus.initial,
        errorMessage: null,
      ));
    }
  }
