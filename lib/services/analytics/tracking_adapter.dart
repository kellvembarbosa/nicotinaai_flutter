...
class TrackingAdapterRegistry {
  static final TrackingAdapterRegistry _instance = TrackingAdapterRegistry._internal();
  ...
  void registerFactory(TrackingAdapterFactory factory) {
    ...
    debugPrint('➕ [TrackingAdapterRegistry] Registered factory for ${factory.adapterName}');
  }
  
  /// Unregister a tracking adapter factory
  void unregisterFactory(String adapterName) {
    _factories.remove(adapterName);
    debugPrint('➖ [TrackingAdapterRegistry] Unregistered factory for $adapterName');
  }
  
  /// Create a tracking adapter
  TrackingAdapter? createAdapter(String adapterName, [TrackingAdapterConfig? config]) {
    final factory = _factories[adapterName];
    if (factory == null) {
      debugPrint('⚠️ [TrackingAdapterRegistry] No factory registered for $adapterName');
      return null;
    }
