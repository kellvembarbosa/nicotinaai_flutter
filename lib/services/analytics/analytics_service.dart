...
class AnalyticsService {
  ...
  Future<void> initialize() async {
    ...
    debugPrint('✅ [AnalyticsService] Initialized with ${_adapters.length} adapters');
    _isInitialized = true;
  }
  
  /// Add a tracking adapter
  Future<bool> addAdapter(String adapterName, {Map<String, dynamic>? config}) async {
  ...
  Future<void> trackEvent(String eventName, {Map<String, dynamic>? parameters}) async {
    ...
    debugPrint('📊 [AnalyticsService] Tracking event: $eventName with parameters: $parameters');
  ...
  Future<void> setUserProperties({
    ...
    if (cigarettesPerDay != null) properties['cigarettes_per_day'] = cigarettesPerDay;
  ...
  void setTrackingEnabled(bool enabled) {
    ...
    debugPrint('🔄 [AnalyticsService] ${enabled ? 'Enabled' : 'Disabled'} tracking for all adapters');
  }
  
  /// Get active adapters
  List<String> get activeAdapters => _adapters.map((adapter) => adapter.adapterName).toList();
  ...
  Future<void> logHealthRecoveryAchieved(String recoveryName) async {
    await trackEvent('health_recovery_achieved', parameters: {'recovery_name': recoveryName});
  }
  
  /// Log craving resisted
  Future<void> logCravingResisted({String? triggerType}) async {
    await trackEvent('craving_resisted', parameters: triggerType != null ? {'trigger': triggerType} : null);
  }
  
  /// Log money saved milestone
  Future<void> logMoneySavedMilestone(double amount, String currency) async {
    await trackEvent('money_saved_milestone', parameters: {
      'amount': amount,
      'currency': currency,
    });
  }
