...
import 'package:nicotinaai_flutter/features/home/widgets/home_screen_skeleton.dart';
...
import 'package:nicotinaai_flutter/widgets/number_skeleton_wrapper.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _initializeBlocs() {
    ...
    if (authState.status == bloc_auth.AuthStatus.authenticated && authState.user != null) {
      ...
      // Definir estado de carregamento inicial
      setState(() {
        _isLoading = true;
      });
    }
  ...
  void _loadData(TrackingState trackingState) {
    ...
    setState(() {
      _isUpdating = true;
      // Manter o estado de carregamento durante o processo
      _isLoading = true;
    });
    ...
    try {
      ...
      if (mounted) {
        ...
        setState(() {
          ...
          // Agora podemos remover o estado de carregamento
          _isLoading = false;
        });
      } else {
        _isUpdating = false;
      ...
      if (mounted) {
        setState(() {
          ...
          _isLoading = false; // Remover estado de carregamento mesmo em caso de erro
        });
      } else {
        _isUpdating = false;
      }
          ...
          return MultiBlocListener(
            listeners: [
              ...
              BlocListener<SmokingRecordBloc, SmokingRecordState>(
                ...
                listener: (context, state) {
                  ...
                  // Atualizar estado de carregamento com base no status do SmokingRecordBloc
                  setState(() {
                    _isLoading = state.status == SmokingRecordStatus.loading || 
                              state.status == SmokingRecordStatus.saving;
                  });
            ...
            child: BlocBuilder<TrackingBloc, TrackingState>(
              builder: (context, trackingState) {
                ...
                // Atualizar estado de carregamento com base no status do TrackingBloc
                if (trackingState.isStatsLoading && !_isLoading) {
                  setState(() {
                    _isLoading = true;
                  });
                }
                                ...
                                Column(
                                  ...
                                  children: [
                                    ...
                                    Column(
                                      crossAxisAlignment: CrossAxisAlignment.start,
                                      children: [
                                        // Usar NumberSkeletonWrapper para mostrar skeleton carregando
                                        NumberSkeletonWrapper<int>(
                                          value: _daysWithoutSmoking,
                                          formatter: (days) => l10n.homeDaysWithoutSmoking(days),
                                          isLoading: _isLoading,
                                          style: context.subtitleStyle,
                                          skeletonHeight: 16,
                                          skeletonWidth: 150,
                                        ),
                                        if (_stats?.lastSmokeDate != null && !_isLoading)
                                          Text(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          // Wrapper de skeleton para o valor numérico
          NumberSkeletonWrapper<String>(
            value: value,
            formatter: (val) => val,
            isLoading: _isLoading,
        ...
        children: [
          Row(
            children: [
              ...
              const Spacer(),
              if (_daysWithoutSmoking > 0 && !_isLoading) ...[
                const Icon(
                  Icons.arrow_upward,
                  color: Colors.green,
                  size: 16,
          ...
          const SizedBox(height: 16),
          // Usar MoneySkeletonWrapper para valores monetários
          MoneySkeletonWrapper(
            valueInCents: valueInCents,
            formatter: (cents) => user == null 
                ? _currencyUtils.formatWithDeviceLocale(cents, context: context)
                : _currencyUtils.format(cents, user: user),
            isLoading: _isLoading,
            style: context.textTheme.headlineMedium!.copyWith(
              fontWeight: FontWeight.bold,
              color: context.contentColor,
