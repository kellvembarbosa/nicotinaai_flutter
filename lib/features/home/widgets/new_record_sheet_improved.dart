...
class NewRecordSheetImproved extends StatefulWidget {
  const NewRecordSheetImproved({super.key});
  ...
  static Future<bool> show(BuildContext context) async {
    final result = await showModalBottomSheet<bool>(
      ...
      builder: (context) => const NewRecordSheetImproved(),
    );
  ...
  @override
  State<NewRecordSheetImproved> createState() => _NewRecordSheetImprovedState();
}

class _NewRecordSheetImprovedState extends State<NewRecordSheetImproved> {
                  ...
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed: !_isFormValid() ? null : _saveRecord,
  ...
  void _saveRecord() async {
    ...
    if (authBloc.state.status != bloc_auth.AuthStatus.authenticated || authBloc.state.user == null) {
    ...
    // Get the record bloc
    final recordBloc = context.read<SmokingRecordBloc>();
    
    // Check for achievements while context is still valid
    try {
      AchievementHelper.checkAfterSmokingRecord(context);
    } catch (e) {
    ...
    // Use ImprovedOptimisticUtils to handle optimistic updates
    final bool success = await ImprovedOptimisticUtils.createRecordOptimistically<SmokingRecordEvent, SmokingRecordState, SmokingRecordModel>(
      context: context,
      bloc: recordBloc,
      createEvent: (record) => SaveSmokingRecordRequested(record: record),
      checkSuccess: (state) => state.status == SmokingRecordStatus.loaded,
      record: record,
      entityName: l10n.newRecord,
    );
