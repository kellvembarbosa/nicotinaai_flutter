...
class _AddSmokingLogScreenState extends State<AddSmokingLogScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Log Smoking Event'),
      ),
            ...
            children: [
              ...
              TextFormField(
                decoration: const InputDecoration(
                  labelText: 'Location',
                  hintText: 'Where were you when smoking?',
              ...
              TextFormField(
                decoration: const InputDecoration(
                  labelText: 'Trigger',
                  hintText: 'What triggered you to smoke?',
                  border: OutlineInputBorder(),
                ),
        ...
        SegmentedButton<ProductType>(
          segments: const [
            ButtonSegment<ProductType>(
              value: ProductType.cigaretteOnly,
              label: Text('Cigarette'),
              icon: Icon(Icons.smoke_free),
            ),
            ButtonSegment<ProductType>(
              value: ProductType.vapeOnly,
              label: Text('Vape'),
              icon: Icon(Icons.air),
            ),
            ButtonSegment<ProductType>(
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Error logging smoking event: $e')),
          );
        }
      } finally {
        if (mounted) {
